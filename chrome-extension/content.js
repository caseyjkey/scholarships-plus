...
function addSparkleIcon(input, fieldName, labelElement, fieldLabel) {
  ...
  // Don't set icon.title - it causes duplicate tooltips with the custom tooltip div
  ...
  icon.addEventListener('click', function(e) {
    ...
    // If sparkle is ready with an approved value, autofill the field
    if (state === 'ready') {
      var mapping = fieldMappings[fieldName];
      if (mapping && mapping.approvedValue) {
        fillFieldValue(input, mapping.approvedValue, icon, fieldName);
        return;
      }
    }

    // Otherwise, open conversation
...
function updateSparkleState(fieldName, isGenerating) {
  ...
  if (isGenerating) {
    ...
    // Don't set icon.title - it causes duplicate tooltips
...
function openConversation(fieldName, input, icon, extractedLabel) {
  ...
  getAuthToken(function(token) {
    ...
    // Check if approvedValue is a placeholder or real value
    var isPlaceholder = mapping && mapping.approvedValue && (
      mapping.approvedValue.includes('[') ||
      mapping.approvedValue.includes('Insert') ||
      mapping.approvedValue.includes('Your answer') ||
      mapping.approvedValue.length < 10
    );

    // If we have a real approvedValue (not a placeholder), show it
    if (mapping && mapping.approvedValue && !isPlaceholder) {
...
/**
 * Fill field value and update sparkle state
 * Called when user clicks a ready sparkle to autofill the field
 */
function fillFieldValue(input, value, icon, fieldName) {
  if (!input || !value) return;

  // Fill the field with animation
  fillFieldWithAnimation(input, value).then(function() {
    // Update sparkle to filled state
    if (icon) {
      icon.classList.remove('sp-sparkle-ready');
      icon.classList.add('sp-sparkle-filled');
      var tooltip = icon.querySelector('.sp-tooltip');
      if (tooltip) {
    ...
    // Optional: play a subtle sound (not implemented yet)
    console.log('[AUTOFILL] Filled field', fieldName, 'with:', value.substring(0, 50));
  });
}
...
function init() {
  ...
  // Clean up any legacy title attributes that cause duplicate tooltips
  // This fixes the issue where Chrome caches old content script versions
  setTimeout(function() {
    ...
    if (cleaned > 0) {
      console.log('[CLEANUP] Removed title attributes from ' + cleaned + ' sparkles (fixes duplicate tooltips)');
    }
  }, 2000);
